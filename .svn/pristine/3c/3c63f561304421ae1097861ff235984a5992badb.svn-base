...
var informationContact = new (function() {
	...
	function fillInfo(response) {
		...
		response.forEach(function(contact) {
					...
					if(!isManager){
						buttonSauver = "<button class=\"buttonUpdateParticipationContact btn btn-default \" id=\"" + contact.id + "\" name=\"bs" + contact.id + "\">Participation</button>";
					}else{
						buttonSauver = "<button class=\"buttonUpdateContact btn btn-default \" id=\"" + contact.id + "\" name=\"bs" + contact.id + "\">Sauver</button>";
					}
					if(!contact.active){
						desactive = "DESACTIVE";
						buttonSauver = "";
	...
	function desactivateContact(elem, contactId){
		...
		var success =  function(response) {
			...
			$(elem).replaceWith( "<div class=\"col-md-1\">DESACTIVE</div>" );
			if(isManager){
				$(document.getElementsByName("bs" + contactId)).replaceWith( "<div class=\"col-md-1 col-md-offset-8\"></div>" );				
			} else {
				$(document.getElementsByName("bs" + contactId)).replaceWith( "<div class=\"col-md-1 col-md-offset-8\"></div>" );
			}
		}
